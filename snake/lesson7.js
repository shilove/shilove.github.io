...
function createObstacle() {
    ...
    while (!obstacleCreated) { //пока препятствие не создали
        ...
        if (!obstacle_cell_classes.includes('snake-unit')&&!obstacle_cell_classes.includes('food-unit')) {
            ...
            obstacle_cell.innerHTML="&#9760;";
            obstacle_cell.style.fontSize="11px";
            obstacleCreated = true;
        }
    }
    setTimeout(removeObstacle, 5000, obstacle_x, obstacle_y);
}

/**
 * Удаление препятствия
 * @param obstacle_x,obstacle_y - координаты
 */
function removeObstacle(obstacle_x,obstacle_y){
    var obstacle_cell = document.getElementsByClassName('cell-' + obstacle_y + '-' + obstacle_x)[0];
    obstacle_cell.classList.remove("obstacle-unit");
    obstacle_cell.innerHTML="";
}
